...
if ( ! class_exists( 'WP_Dependency_Installer' ) ) {
	...
	class WP_Dependency_Installer {
				...
				if ( $this->is_installed( $slug ) ) {
					if ( $is_optional ) {
						$this->notices[] = array(
							'action' => 'activate',
							'slug'   => $slug,
							'text'   => sprintf( __( 'Attivare il plugin %s.' ), $dependency['name'] ),
						);
					...
					$this->notices[] = array(
						'action' => 'install',
						'slug'   => $slug,
						'text'   => sprintf( __( 'Il plugin %s è richiesto.' ), $dependency['name'] ),
					);
				} else {
		...
		public function install( $slug ) {
			...
			if ( ! $this->config[ $slug ]['optional'] ) {
				...
				return array(
					'status'  => 'updated',
					'slug'    => $slug,
					'message' => sprintf( __( '%s è stato installato e attivato.' ), $this->config[ $slug ]['name'] ),
				);
			...
			return array(
				'status'  => 'updated',
				'message' => sprintf( __( '%s è stato installato.' ), $this->config[ $slug ]['name'] ),
			);
		}
		...
		public function activate( $slug ) {
			...
			return array(
				'status'  => 'updated',
				'message' => sprintf( __( '%s è stato attivato.' ), $this->config[ $slug ]['name'] ),
			);
		}
