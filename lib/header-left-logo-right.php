...
function atp_new_header_class( $attributes ) {
 $attributes['class'] = 'site-header header-left-logo-right';
return $attributes;
}
...
genesis_register_widget_area(
    array(
        'id'          => 'header-left',
        'name'        => __( 'Header Left', 'my-theme-text-domain' ),
        'description' => __( 'This is the header left widget area', 'my-theme-text-domain' ),
    )
);
 
// Add "one-third"  class to .title-area
...
/**
 * Echo the default header, including the #title-area div, along with #title and #description, as well as the left and right widget areas.
 *
...
function sk_do_header() {
    ...
    genesis_widget_area( 'header-left', array(
        'before'    => '<div class="widget-area header-left one-third first">',
        'after'     => '</div>',
    ) );
